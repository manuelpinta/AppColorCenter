...
import { Building2, MapPin, ChevronRight, ChevronLeft, Users } from "lucide-react"
...
export default function SucursalesPage() {
  ...
  // Stats por empresa
  const empresaStats = mockEmpresas.map((empresa) => {
    const sucursales = mockColorCenters.filter((c) => c.empresa_id === empresa.id)
  ...
  return (
    <div className="pb-20 lg:pb-0">
      <div className="px-4 py-6 lg:px-8 lg:py-8 max-w-7xl mx-auto">
        <div className="mb-6">
          <h1 className="text-2xl lg:text-3xl font-bold text-foreground tracking-tight">Sucursales</h1>
          <p className="text-sm lg:text-base text-muted-foreground mt-1">
            {mockColorCenters.length} sucursales en {mockEmpresas.length} empresas
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
          {empresaStats.map((empresa) => (
            <Card 
              ...
              onClick={() => {
                setEmpresaFilter(empresaFilter === empresa.id ? "all" : empresa.id)
                setRegionFilter("all")
              }}
            >
              <CardContent className="p-4">
                ...
                <div className="flex items-center justify-between">
                  <span className="text-2xl font-bold">{empresa.total}</span>
                  <span className="text-xs text-emerald-600">{empresa.operativas} op.</span>
                </div>
              </CardContent>
            </Card>
          ))}
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b bg-muted/50">
                ...
                <th className="text-left py-3 px-4 text-xs font-medium text-muted-foreground uppercase tracking-wide hidden lg:table-cell">Responsable</th>
                  ...
                  <tr key={center.id} className="hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-3 px-4 hidden lg:table-cell">
                      <div className="flex items-center gap-1.5 text-sm text-muted-foreground">
                        ...
                        <span className="truncate max-w-[120px]">{center.responsable || "-"}</span>
                      </div>
                    </td>
                    <td className="py-3 px-4">
